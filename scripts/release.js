...
1. Make sure you have publish access for all packages:
    - You must be in the CLI team in the npm @kdujs organization
    - You must have publish access to kdu-cli-version-marker
...
const release = async () => {
  ...
  const versionMarkerPath = path.resolve(__dirname, '../packages/kdu-cli-version-marker/package.json')
  ...
  const tagName = `kdu-cli-version-marker@${versionMarkerPkg.version}`
  await execa('git', ['add', '-A'], { stdio: 'inherit' })
